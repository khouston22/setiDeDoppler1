...
% 
  ...
  if (~exist('N0','var')),  N0 = 8; end
  if isempty(N0),           N0 = 8; end

  if (~exist('alg_ID','var')),  alg_ID = 1; end
  if isempty(alg_ID),           alg_ID = 1; end

  [n_freq1,n_time] = size(xx);
  if length(freq1)~=n_freq1
    error(sprintf('Error in fastDD3, xx and freq1 sizes incompatible, %.0f vs %.0f\n',...
      n_freq1,length(freq1)));
  end

  if Nt~=N0*2.^round(log2(Nt/N0))
    error(sprintf('Error in fastDD3, Nt/N0 not a power of 2, %.0f\n',Nt));
  end
  
  if (Lr>Lf)
    fprintf('Warning in fastDD3, Lr=%.0f cannot be greater than Lf=%.0f\n',...
      Lr,Lf);
    Lr = Lf;
  end
  if mod(Lf,Lr)~=0
    error(sprintf('Error in fastDD3, Lf=%.0f must be multiple of Lr=%.0f\n',...
      Lf,Lr));
  end
  ...
  if (alg_ID>=1) % fastDD variants
    m_max0 = ceil(df_dt_max*T_line/fs*Lr*N0);
    m_min0 = floor(df_dt_min*T_line/fs*Lr*N0);
    m_max = round(m_max0*Nt/N0);
    m_min = round(m_min0*Nt/N0);
    m_list  = [m_min:m_max];
    df_dt_list = m_list*fs/T_line/(Nt*Lr);
  else  % Taylor DD
    m_max0 = ceil(df_dt_max*T_line/fs*(Lr*N0-1));
    m_min0 = floor(df_dt_min*T_line/fs*(Lr*N0-1));
    [m_min,m_max] = m_limit(m_min0,m_max0,Nt,N0);
    m_list  = [m_min:m_max];
    df_dt_list = m_list*fs/T_line/(Nt*Lr-1);
  end
  ...
  if (alg_ID>0)
    ...
    det_DD_i = fastDD_stage1(xx,n_freq1,Lf,Lr,Nt,N0,m_min0,m_max0,alg_ID,dd0);

    % fastDD variants
    for i_stage = 2:n_stage
      det_DD_i = fastDD_stage_i(det_DD_i,i_stage,n_freq2,Nt,N0,m_min0,m_max0,alg_ID,dd0);
    end
  else
    % first stage processing
...
function det_DD1 = fastDD_stage1(xx,n_freq1,Lf,Lr,Nt,N0,m_min0,m_max0,alg_ID,dd0)
  %
  % "Slow" first stage
  %
  Nr0 = m_max0 - m_min0 + 1;
  if (alg_ID>=1) % fastDD variants
    freq_bin_offset = Lf/Lr*[m_min0:m_max0]'*[0:N0-1]/(N0);
  else  % Taylor DD
    freq_bin_offset = Lf/Lr*[m_min0:m_max0]'*[0:N0-1]/(N0-1);
  end
  int_bin_offset = round(freq_bin_offset);
...
function det_DD_out = fastDD_stage_i(det_DD_in,i_stage,n_freq2,Nt,N0,m_min0,m_max0,alg_ID,dd0)
  ...
  m_max_out = m_max0*N_out/N0; % drift rate index limits - current stage
  m_min_out = m_min0*N_out/N0;
  m_max_in = m_max0*N_in/N0;   % drift rate index limits - previous stage
  m_min_in = m_min0*N_in/N0;
  ...
  for ig = 1:n_group
    ...
    for i = 1:Nr_in
      ...
      k_ofs2 = dd0 + m + 2;
      % "even" output points
      det_DD_out(dd0+1:dd0+n_freq2,m02+2*i-1) = ...
                             det_DD_in(dd0+1:dd0+n_freq2,m00+i) + ...
                             det_DD_in(k_ofs0+1:k_ofs0+n_freq2,m01+i);
      if (i<Nr_in)
        % "odd" output points
        if (alg_ID==1)
        % baseline = point-by-point max of two dog-leg sums
          det_DD_out(dd0+1:dd0+n_freq2,m02+2*i) = max(...
          det_DD_in(dd0+1:dd0+n_freq2,m00+i)  +det_DD_in(k_ofs0+1:k_ofs0+n_freq2,m01+i+1),...
          det_DD_in(dd0+1:dd0+n_freq2,m00+i+1)+det_DD_in(k_ofs1+1:k_ofs1+n_freq2,m01+i));
        elseif (alg_ID==2)
        % cubic interpolation
          if (i==1)
            det_DD_out(dd0+1:dd0+n_freq2,m02+2*i) = ...
               .375*det_DD_in(dd0+1:dd0+n_freq2,m00+i) + ...
               .750*det_DD_in(dd0+1:dd0+n_freq2,m00+i+1) + ...
              -.125*det_DD_in(dd0+1:dd0+n_freq2,m00+i+2) + ...
               .375*det_DD_in(k_ofs0+1:k_ofs0+n_freq2,m01+i+1) + ...
               .750*det_DD_in(k_ofs1+1:k_ofs1+n_freq2,m01+i);
          elseif (i==Nr_in-1)
            det_DD_out(dd0+1:dd0+n_freq2,m02+2*i) = ...
               .375*det_DD_in(dd0+1:dd0+n_freq2,m00+i) + ...
               .750*det_DD_in(dd0+1:dd0+n_freq2,m00+i+1) + ...
               .375*det_DD_in(k_ofs0+1:k_ofs0+n_freq2,m01+i+1) + ...
               .750*det_DD_in(k_ofs1+1:k_ofs1+n_freq2,m01+i) + ...
              -.125*det_DD_in(k_ofs2+1:k_ofs2+n_freq2,m01+i-1);
          else
            det_DD_out(dd0+1:dd0+n_freq2,m02+2*i) = ...
               .375*det_DD_in(dd0+1:dd0+n_freq2,m00+i) + ...
               .750*det_DD_in(dd0+1:dd0+n_freq2,m00+i+1) + ...
        ...
        % lower dog leg
          det_DD_out(dd0+1:dd0+n_freq2,m02+2*i) = ...
              det_DD_in(dd0+1:dd0+n_freq2,m00+i) + ...
              det_DD_in(k_ofs0+1:k_ofs0+n_freq2,m01+i+1);
        elseif (alg_ID==4)
        % upper dog leg
          det_DD_out(dd0+1:dd0+n_freq2,m02+2*i) = ...
              det_DD_in(dd0+1:dd0+n_freq2,m00+i+1) + ...
              det_DD_in(k_ofs1+1:k_ofs1+n_freq2,m01+i);
        elseif (alg_ID==5)
        ...
        % lower leg then upper
          ...
          error(sprintf('Error in fastDD3, alg_ID=%.0f not defined',alg_ID));
        end
      end
    end    
  end
