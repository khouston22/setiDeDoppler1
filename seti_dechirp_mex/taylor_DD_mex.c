...
{ 
  ...
  #if DEBUG
    mexPrintf("Checking inputs,n_freq1=%ld,n_freq2=%ld,Nt=%ld,N0=%ld,Lf=%.2f,Lr=%.2f,LfLr_ratio=%ld\n",
           n_freq1,n_freq2,Nt,N0,Lf,Lr,LfLr_ratio);
    mexPrintf("n_freq1=%ld vs %ld, %ld x %ld\n",n_freq1,n_freq1_,n_freq1_r,n_freq1_c);
  #endif

  if (fabs(LfLr_ratio*Lr-Lf)>1e-5) { 
    mexErrMsgIdAndTxt( "MATLAB:taylor_DD:invalidLfLr_ratio",
