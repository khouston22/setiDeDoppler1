function [det_DD,df_dt_list,m_list] = taylorDD0_c(xx,freq1,t_pfb,N,pos_neg);
%
...
%
  ...
  if (pos_neg>=0)
    ...
    outbuf = taylor_flt(inbuf,mlen,N)';
    out_p = reshape(outbuf,n_freq1x,[])/N;
  ...
  if (pos_neg<=0)
    ...
    outbuf = taylor_flt(inbuf,mlen,N)';
    out_n = fliplr(flipud(reshape(outbuf,n_freq1x,[])))/N;
